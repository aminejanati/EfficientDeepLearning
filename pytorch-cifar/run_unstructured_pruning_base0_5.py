...
def save_plots(sweep_accuracies, histories, epochs, plot_dir):
    ...
    plt.ylabel("Accuracy")
    plt.title("Model Accuracy vs Unstructured Weight Pruning Amount (Base Model 0.5 Structured Pruned)")
    plt.grid(True)
    ...
    plt.title("Unstructured Pruning: Accuracy over Fine-tuning (Pretrained Base Model 0.5 Structured Pruned)")
    plt.grid(True)
    plt.legend()
    ...
    plt.title("Unstructured Pruning: Loss over Fine-tuning (Pretrained Base Model 0.5 Structured Pruned)")
    plt.grid(True)
    plt.legend()
...
def run(args):
    ...
    for prune_amount in args.finetune_amounts:
        ...
        save_name = (
            ...
            f"basemodel_0.5structured_augmix_{args.epochs}ep"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Run unstructured pruning + finetuning (base model: structured-pruned 0.5)"
    )
    parser.add_argument(
        "--checkpoint",
        type=str,
        default="Models Weights/Resnet18_structured_pruned_50_augmix_25ep.pth",
    ...
    parser.add_argument(
        "--output-json",
        type=str,
        default="unstructured_pruning_base0_5_results.json",
        help="output summary json path",
    )
    parser.add_argument(
        "--plot-dir",
        type=str,
        default="Model Plots/Resnet18_unstructured_pruned_basemodel_0.5structured_augmix",
