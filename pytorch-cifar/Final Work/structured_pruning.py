"""Run structured pruning on a stored distilled CIFAR-10 model."""
...
# ----------------------
CHECKPOINT_PATH = "Final Weights/step1/resnet18_depthwise.pth"
MODEL_ARCH = "resnet18_depthwise"
PRUNING_AMOUNT = 0.7
PRUNING_N = 2
PRUNING_DIM = 0  # dim=0 prunes output channels (filters)
FINETUNE_EPOCHS = 30
FINETUNE_LR = 0.01
...
NUM_WORKERS = 2
DATA_DIR = "data"
RESULTS_JSON = "Final Weights/step2/structured_pruning_distilled_results.json"
SAVE_DIR = "Final Weights/step2"
...
def apply_structured_pruning(model: nn.Module, amount: float, n: int, dim: int):
...
def count_zeroed_filters(model: nn.Module):
    zeroed = 0
    total = 0
    for module in model.modules():
        if isinstance(module, nn.Conv2d):
            ...
            flattened = weights.view(out_channels, -1)
            zeroed += (flattened.abs().sum(dim=1) == 0).sum().item()
            total += out_channels
...
def main():
    ...
    testloader = build_test_loader(data_dir, BATCH_SIZE, NUM_WORKERS)
    ...
    apply_structured_pruning(pruned_model, amount=amount, n=PRUNING_N, dim=PRUNING_DIM)
    pruned_acc_before_ft = evaluate(pruned_model, testloader, device)
    zeroed_filters, total_filters = count_zeroed_filters(pruned_model)
    zeroed_ratio = (100.0 * zeroed_filters / total_filters) if total_filters else 0.0
    ...
    print(
        ...
        f"zeroed_filters={zeroed_filters}/{total_filters} ({zeroed_ratio:.2f}%)"
    )

    save_dir.mkdir(parents=True, exist_ok=True)
    suffix = str(amount).replace(".", "p")
    ckpt_name = f"{checkpoint_path.stem}_structured_{suffix}_finetuned.pth"
