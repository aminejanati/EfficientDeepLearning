...
DEFAULT_INT8_DIR = PROJECT_ROOT / "Models Weights" / "quantization" / "unstructured_pruning_ptq"
DEFAULT_RESULTS_JSON = PROJECT_ROOT / "quantization" / "unstructured_pruning_ptq_results.json"


def parse_args():
    parser = argparse.ArgumentParser(
        description="Post-training INT8 quantization (no fine-tuning) for all unstructured-pruned checkpoints."
    )
    parser.add_argument("--weights-dir", type=Path, default=DEFAULT_WEIGHTS_DIR)
    parser.add_argument("--glob", type=str, default="*.pth")
    parser.add_argument("--output-dir", type=Path, default=DEFAULT_INT8_DIR)
...
def ptq_convert_no_finetune(fp32_model, calib_loader, backend, calib_batches=100):
...
def process_checkpoint(checkpoint_path, calib_loader, test_loader, eval_device, backend, output_dir, calib_batches):
    ...
    int8_model = ptq_convert_no_finetune(fp32_model, calib_loader, backend, calib_batches=calib_batches)
    int8_loss, int8_acc = evaluate_model(int8_model, test_loader, "cpu")
    print(f"  INT8 PTQ Test -> loss: {int8_loss:.4f}, acc: {int8_acc:.2f}%")

    save_name = checkpoint_path.stem + "_int8_ptq_nofinetune.pth"
    save_path = output_dir / save_name
    torch.save(
        {
            "net": int8_model.state_dict(),
            "source_checkpoint": str(checkpoint_path),
            "quantization": "PTQ INT8 (no fine-tuning)",
            "backend": backend,
    ...
    return {
        ...
        "int8_checkpoint": save_name,
        "fp32_test_loss": round(fp32_loss, 6),
        "fp32_test_acc": round(fp32_acc, 4),
        "int8_test_loss": round(int8_loss, 6),
        "int8_test_acc": round(int8_acc, 4),
        "acc_drop": round(fp32_acc - int8_acc, 4),
    }
...
def main():
    ...
    if results:
        ...
        avg_int8 = sum(r["int8_test_acc"] for r in results) / len(results)
        avg_drop = avg_fp32 - avg_int8
        print(f"Average FP32 acc : {avg_fp32:.2f}%")
        print(f"Average INT8 acc : {avg_int8:.2f}%")
