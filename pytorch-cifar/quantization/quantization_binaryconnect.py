...
if args.resume:
    ...
    assert checkpoint_dir.is_dir(), 'Error: no checkpoint directory found!'
    checkpoint = torch.load(str(checkpoint_dir / 'ckpt.pth'), map_location=device)
...
weights_dir = PROJECT_ROOT / 'Models Weights' / 'quantization'
...
save_path = weights_dir / f'{save_stem}.pth'
torch.save(best_state, str(save_path))
torch.save(best_state, str(checkpoint_dir / 'ckpt.pth'))

plots_root_dir = PROJECT_ROOT / 'Model Plots' / 'quantization'
plots_root_dir.mkdir(parents=True, exist_ok=True)

model_plot_dir = plots_root_dir / save_stem
...
plt.tight_layout()
plot_path = model_plot_dir / 'training_history.png'
